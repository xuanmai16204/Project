...
namespace WebsiteDienNha_DoAnChuyenNganh.Controllers
{
	public class AccountController : Controller
	{
		...
		public async Task<IActionResult> Login(LoginViewModel model, string? returnUrl = null)
		{
			...
			if (!passwordValid)
			{
				// If password is invalid, try to reset it if it seems like it might be an old/unhashed password
				// This is a fallback for users that might have been created manually
				var hasPasswordHash = await _userManager.HasPasswordAsync(user);
				if (!hasPasswordHash)
				{
					// User doesn't have a password set, try to set it
			...
			// Redirect admin to admin dashboard (bỏ qua returnUrl)
			if (await _userManager.IsInRoleAsync(user, SD.RoleAdmin))
			{
				return RedirectToAction("Index", "Home", new { area = "Admin" });
			}
			
			// Chỉ redirect đến returnUrl nếu không phải admin
			return RedirectToLocal(returnUrl);
		}
		...
		[HttpGet]
		public async Task<IActionResult> ResetPassword(string email, string newPassword = "123456")
		{
			// This is a development helper - should be removed or secured in production
			...
			if (result.Succeeded)
			{
				return Json(new { success = true, message = $"Password reset to {newPassword} for {email}" });
			}
